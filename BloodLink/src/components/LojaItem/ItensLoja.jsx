import mainSet from '../../assets/SetBloomii1.png';
import alegriaMii from '../../assets/AlegriaMii.png';
import neveMii from '../../assets/NeveMii.png';
import magiaMii from '../../assets/MagiaMii.png';
import alegriaGif from '../../assets/SetBloomii11.gif';
import neveGif from '../../assets/SetBloomii12.gif';
import magiaGif from '../../assets/SetBloomii13.gif';

import set2 from '../../assets/HoraDoChazinho.png';
import chaGif from '../../assets/chazinho.gif';

import set3 from '../../assets/PijaminhaMon.png';
import pijaminha from '../../assets/PijaminhaMon1.png';

import leitura from '../../assets/Leitura.png';
import lendo from '../../assets/Lendo.png';

import medik from '../../assets/Medik.png';
import medikMii from '../../assets/Medi1.png';

import bloomii from '../../assets/Bloomiin.png';
import bloomiin from '../../assets/Bloomiin1.png';

import gameOn from '../../assets/gameOn.png';
import gameGif from '../../assets/Bloomii-Setgamer.gif';

import Linky from '../../assets/Linky.png';
import LinkyGif from '../../assets/LinkyI.gif';

import face1 from '../../assets/rosto1.png';
import seriedade from '../../assets/seriedade.png';

import face2 from '../../assets/rosto2.png';
import blomimir from '../../assets/bloomimir.png';

import cabelo1 from '../../assets/cabeloI1.png';
import punk from '../../assets/punk.png';

import cabelo2 from '../../assets/cabeloI2.png';
import rabinho from '../../assets/rabinho.png';

import camisa1 from '../../assets/camisa1.png';
import regatinha from '../../assets/regatinha.png';

import camisa2 from '../../assets/camisa2.png';
import blusao from '../../assets/blusao.png';

import camisa3 from '../../assets/camisa3.png';
import social from '../../assets/social.png';

import calca1 from '../../assets/calca1.png';
import bermas from '../../assets/bermas.png';

import calca2 from '../../assets/calca2.png';
import saia from '../../assets/saia.png';

import calca3 from '../../assets/calca3.png';
import estiloso from '../../assets/estiloso.png';

import acessorio1 from '../../assets/acessorio1.png';
import oclinhos from '../../assets/oculos.png';

import acessorio2 from '../../assets/acessorio2.png';
import reis from '../../assets/reizinho.png';

import acessorio3 from '../../assets/acessorio3.png';
import cicatriz from '../../assets/cicatriz.png';

import acessorio4 from '../../assets/acessorio4.png';
import tapaolho from '../../assets/tapaolho.png';

import bloomi1 from '../../assets/Bloomii1.png';
import bloomiiIdle from '../../assets/Bloomii1.gif';

import bloomi2 from '../../assets/Bloomii2.png';
import bloomiiforte from '../../assets/Bloomii2.gif';

import bloomi3 from '../../assets/Bloomii3.png';
import bloomiifeliz from '../../assets/Bloomii3.gif';

import bloomi4 from '../../assets/Bloomii4.png';
import bloomiilinky from '../../assets/Bloomii4.gif';

import bloomiuniforme from '../../assets/BloomiiUniforme.png';
import bloomiiC from '../../assets/BloodLinkUniforme.png';


import proa1 from '../../assets/proa1.png';
import jailson from '../../assets/jaja.gif';

import proa2 from '../../assets/proa2.png';
import aurora from '../../assets/aurora.gif';

import proa3 from '../../assets/proa3.png';
import julio from '../../assets/julio.gif';

import proa4 from '../../assets/proa4.png';
import manto1 from '../../assets/manto1.png';
import proa5 from '../../assets/proa5.png';
import manto2 from '../../assets/manto2.png';

export const itensLoja = [
    {
        id: '1',
        nome: 'Set Inverno Mágico',
        preco: 1050,
        imagem: mainSet,
        descricao: 'Um set encantado para os dias frios! *Contém três Bloomiis animados.',
        tipo: 'set',
        partes: [alegriaMii, neveMii, magiaMii,],
        previews: [alegriaGif, neveGif, magiaGif]
    },
    {
        id: '2',
        nome: 'Hora de Relachá',
        preco: 350,
        imagem: set2,
        descricao: 'Perfeito para um chá da tarde com estilo. *Contém um Bloomii animado',
        previews: [chaGif]
    },
    {
        id: '3',
        nome: 'Pijaminhamon',
        preco: 150,
        imagem: set3,
        descricao: 'Fofura e conforto para boas noites. *Contém item de categoria camiseta para o Bloomii',
        tipo: 'set',
        previews: [pijaminha]
    },
    {
        id: '4',
        nome: 'AlegriaMii',
        preco: 350,
        imagem: alegriaMii,
        descricao: 'Transborde felicidade e friozinho! *Contém um Bloomii animado',
        previews: [alegriaGif]
    },
    {
        id: '5',
        nome: 'NevinhaMii',
        preco: 350,
        imagem: neveMii,
        descricao: 'Um toque geladinho no estilo! *Contém um Bloomii animado',
        previews: [neveGif]
    },
    {
        id: '6',
        nome: 'MagiaMii',
        preco: 350,
        imagem: magiaMii,
        descricao: 'Brilhe com sua magia interior nesse friozinho! *Contém um Bloomii animado',
        previews: [magiaGif]
    },
    {
        id: '7',
        nome: 'Leitura',
        preco: 250,
        imagem: leitura,
        descricao: 'Ideal para quem ama livros. *Contém um Bloomii',
        previews: [lendo]
    },
    {
        id: '8',
        nome: 'Medik',
        preco: 250,
        imagem: medik,
        descricao: 'Um toque de cuidado no visual. *Contém um Bloomii',
        previews: [medikMii]
    },
    {
        id: '9',
        nome: 'Bloomiin',
        preco: 250,
        imagem: bloomii,
        descricao: 'Estilo casual e moderno para os Bloomiis. *Contém um Bloomii',
        previews: [bloomiin]
    },

];
export const itensMiniLojinha = {
    set: [
        {
            id: '4',
            nome: 'AlegriaMii',
            preco: 350,
            imagem: alegriaMii,
            descricao: 'Transborde felicidade e friozinho! *Contém um Bloomii animado',
            previews: [alegriaGif]
        },
        {
            id: '5',
            nome: 'NevinhaMii',
            preco: 350,
            imagem: neveMii,
            descricao: 'Um toque geladinho no estilo! *Contém um Bloomii animado',
            previews: [neveGif]
        },
        {
            id: '6',
            nome: 'MagiaMii',
            preco: 350,
            imagem: magiaMii,
            descricao: 'Brilhe com sua magia interior nesse friozinho! *Contém um Bloomii animado',
            previews: [magiaGif]
        },
        {
            id: '10',
            nome: 'GameON',
            preco: 350,
            imagem: gameOn,
            descricao: 'Bora jogar! *Contém um Bloomii animado',
            previews: [gameGif]
        },
        {
            id: '11',
            nome: 'Linky',
            preco: 350,
            imagem: Linky,
            descricao: 'A incrível assistente Linky mais pertinho de você. *Contém um Bloomii animado',
            previews: [LinkyGif]
        },
    ],
    rosto: [
        {
            id: '12',
            nome: 'Seriedade',
            preco: 200,
            imagem: face1,
            descricao: 'Traz um tom de seriedade para o Bloomii *Contém item de categoria rosto para o Bloomii ',
            tipo: 'rosto',
            previews: [seriedade]
        },
        {
            id: '13',
            nome: 'Bloomimir',
            preco: 200,
            imagem: face2,
            descricao: 'Para Bloomiis preguiçosos *Contém item de categoria rosto para o Bloomii',
            tipo: 'rosto',
            previews: [blomimir]
        },
    ],
    cabelo: [
        {
            id: '15',
            nome: 'Estilo Punk',
            preco: 220,
            imagem: cabelo1,
            descricao: 'Desperte o lado rebelde do seu Bloomii *Contém item de categoria cabelo para o Bloomii',
            tipo: 'cabelo',
            previews: [punk]
        },
        {
            id: '16',
            nome: 'Rabo de Cavalo',
            preco: 220,
            imagem: cabelo2,
            descricao: 'Clássico e simples, geral ama esse estilo! *Contém item de categoria cabelo para o Bloomii',
            tipo: 'cabelo',
            previews: [rabinho]
        },

    ],

    camisa: [

        {
            id: '3',
            nome: 'Pijaminhamon',
            preco: 150,
            imagem: set3,
            descricao: 'Fofura e conforto para boas noites. *Contém item de categoria camiseta para o Bloomii',
            tipo: 'camisa',
            previews: [pijaminha]
        },
        {
            id: '17',
            nome: 'Regatinha Amarela',
            preco: 200,
            imagem: camisa1,
            descricao: 'Para dias quentes! *Contém item de categoria camisa para o Bloomii',
            tipo: 'camisa',
            previews: [regatinha]
        },
        {
            id: '18',
            nome: 'BlusãoLink',
            preco: 200,
            imagem: camisa2,
            descricao: 'Confortável e grande! *Contém item de categoria camisa para o Bloomii',
            tipo: 'camisa',
            previews: [blusao]
        },
        {
            id: '19',
            nome: 'Regatinha Amarela',
            preco: 200,
            imagem: camisa3,
            descricao: 'Traz um ar de elegância pro Bloomii. *Contém item de categoria camisa para o Bloomii',
            tipo: 'camisa',
            previews: [social]
        },
    ],
    calca: [
        {
            id: '20',
            nome: 'Bermas',
            preco: 200,
            imagem: calca1,
            descricao: 'Bermas para aqueles que precisam! bermas para todos!! *Contém item de categoria calça para o Bloomii',
            tipo: 'calca',
            previews: [bermas]
        },
        {
            id: '21',
            nome: 'Saia Branquinha',
            preco: 200,
            imagem: calca2,
            descricao: 'Saia tão branca quanto a neve! *Contém item de categoria calça para o Bloomii',
            tipo: 'calca',
            previews: [saia]
        },
        {
            id: '22',
            nome: 'No estilo',
            preco: 200,
            imagem: calca3,
            descricao: 'Calça super descolada. Geral tá de olho! *Contém item de categoria calça para o Bloomii',
            tipo: 'calca',
            previews: [estiloso]
        },


    ],
    acessorios: [
        {
            id: '23',
            nome: 'Maneiro até demais',
            preco: 150,
            imagem: acessorio1,
            descricao: 'Óculos escuro maneirasso, da pra sentir a aura saindo desse item... *Contém item de categoria acessório para o Bloomii',
            tipo: 'acessorio',
            previews: [oclinhos]
        },
        {
            id: '244',
            nome: 'Reizinho',
            preco: 150,
            imagem: acessorio2,
            descricao: 'Para os Bloomiis Big boss! *Contém item de categoria acessório para o Bloomii',
            tipo: 'acessorio',
            previews: [reis]
        },
        {
            id: '25',
            nome: 'Cicatriz misteriosa',
            preco: 150,
            imagem: acessorio3,
            descricao: 'Pequena cicatriz. Diz a lenda que um lendário pirata possui essa mesma cicatriz. *Contém item de categoria acessório para o Bloomii',
            tipo: 'acessorio',
            previews: [cicatriz]
        },
        {
            id: '24',
            nome: 'Tapa olho',
            preco: 150,
            imagem: acessorio4,
            descricao: 'O Bloomii que vendeu o mundo tem posse desse item. *Contém item de categoria acessório para o Bloomii',
            tipo: 'acessorio',
            previews: [tapaolho]
        },
    ]
};

export const setsSecretos = [
    {
        id: 's1',
        nome: 'Set Especial BloodLink',
        preco: 1400,
        imagem: bloomi4,
        descricao: 'Set especial com os Bloomiis que fazem a Vitalink existir! *Contém quatro Bloomiis animados e um item de categoria camisa',
        tipo: 'set',
        partes: [bloomi1, bloomi2, bloomi3, bloomi4, bloomiuniforme],
        previews: [bloomiiIdle, bloomiiforte, bloomiifeliz, bloomiilinky, bloomiiC]
    },
    {
        id: 's2',
        nome: 'Set Especial PROA',
        preco: 1900,
        imagem: proa1,
        descricao: 'Set especial do Instituto Proa, com os professores que ajudaram na jornada da BloodLink. *Contém três Bloomiis animados e dois itens de categoria camiseta especiais',
        tipo: 'set',
        partes: [proa1, proa2, proa3, proa4, proa5],
        previews: [jailson, aurora, julio, manto1, manto2]
    },
];